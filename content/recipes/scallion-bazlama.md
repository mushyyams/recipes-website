---
title: "Scallion Bazlama"
excerpt: "Soft Turkish flatbread flecked with green onion — mixed by hand in stages so the dough stays relaxed and the oil fully incorporates before a long proof."
category: "Breakfast"
tags: ["bread", "turkish", "vegetarian", "by-hand"]
prepTime: "2 hr 30 min"
cookTime: "30 min"
servings: 6
difficulty: "Medium"
featured: false
publishedAt: "2026-06-12"
image: "https://images.unsplash.com/photo-1509440159596-0249088772ff?w=1600&q=80"
imageAlt: "Fresh flatbread with herbs on a wooden board"
ingredients:
  - "Dry Ingredients"
  - "250g flour (1 half-quart container of loosely packed flour)"
  - "7g instant dry yeast (one packet)"
  - "5g sugar (1 1/4 tsp)"
  - "Wet Ingredients"
  - "160 ml milk (1/2 of a half-quart container)"
  - "Salt and Oil"
  - "4g salt (1 1/4 tsp)"
  - "9g olive oil (2 tsp)"
  - "Fixings"
  - "1 bunch green onion"
steps:
  - "Mix the dry ingredients in a bowl."
  - "Add the wet ingredients and mix roughly until a shaggy dough forms."
  - "Cover the bowl with a tray or wet towel and rest for 15 minutes."
  - "Mix in the salt, oil, and fixings. Knead until the oil is fully incorporated — the dough surface should change from a shiny sheen to matte."
  - "Cover the bowl with a tray or wet towel and rest for 15 minutes."
  - "Transfer the dough to a lightly floured work surface and divide into 6 equal pieces (one per serving)."
  - "Form each piece into a ball, lightly dust the tops, cover, and proof for 1–2 hours."
  - "Roll each ball out to ¼ inch thickness."
  - "Heat a skillet over medium heat and cook the flattened dough one at a time, flipping and cooking 2–3 minutes per side."
  - "Drizzle with oil or butter and serve."
---

These are the hand-mixed dough stages for scallion bazlama — built in two short rests so the flour hydrates before the salt, oil, and green onion go in. The matte finish on the dough is your signal that the oil is worked in.

## On the rests

The first 15-minute rest lets the shaggy dough relax without overworking it. The second rest, after kneading, gives the gluten a breather before you divide and shape.

## Proofing

One to two hours at room temperature is the window here. The balls should look puffy and relaxed before you roll them out and cook.
